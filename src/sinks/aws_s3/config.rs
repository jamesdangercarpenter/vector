...
impl S3SinkConfig {
    ...
    pub fn build_healthcheck(&self, client: S3Client) -> crate::Result<Healthcheck> {
        if self.verify_write_permission {
            ...
            // The marker is sent with the sink's real options so it is subject to the
            // same bucket policy as application writes. A templated KMS key cannot be
            // rendered without an event, so the marker then relies on the bucket's
            // default encryption instead.
            options.ssekms_key_id = options.ssekms_key_id.filter(|key_id| {
                Template::try_from(key_id.as_str()).is_ok_and(|template| !template.is_dynamic())
            });
